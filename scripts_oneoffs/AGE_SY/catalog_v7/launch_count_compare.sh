#!/bin/bash
# Gate step (runs as a job after the catalog build): count the samples through the
# STANDARD pipeline (merge works now, #24), then chain the v3-vs-v7 comparison.
...
sbatch --dependency=afterok:"$JID" -A tdlong_lab -p standard \
  --cpus-per-task=4 --mem-per-cpu=6G --time=00:40:00 -o logs/AGE_SY/compare_v3_v7.out \
  --wrap="module load R/4.2.2; Rscript pipeline/scripts/compare_refalt_calls.R $V3DIR $DIR/Calls"
echo "compare chained after $JID -> logs/AGE_SY/compare_v3_v7.out"
