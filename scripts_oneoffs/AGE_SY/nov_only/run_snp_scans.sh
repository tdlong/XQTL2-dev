...
for s in "${SCANS[@]}"; do
  out=$(bash pipeline/scripts/run_snp_scan.sh \
          ...
          --scan    "${s}_no89" \
          --parfile "$PARFILE")
  echo "   $(printf '%-22s' "${s}_no89") $(jobid_from "$out")"
done

cat <<EOF

------------------------------------------------------------------
4 SNP scans submitted ->
  $DIR/Scans/<scan>_no89/<scan>_no89.snp_scan.txt
  columns: chr, pos, Wald_log10p, cM, n_informative_founders
------------------------------------------------------------------
EOF
