...
The partition is computed at every step, and Figure 1C shows the three components along the genome. A genome-wide summary needs units that are roughly independent, which steps are not, the frequencies entering them being smoothed over ±100 kb; and mapping resolution is genetic rather than physical. The map was therefore cut into *tiles* of 1 cM from the start of each arm, 268 of them covering the 264 cM and 122 Mb of euchromatin. A tile is read at a single step, the one within it whose Wald statistic, maximised over the four treatments, is largest, so that all eight estimates refer to the same place. Where nothing has responded the components are all small and their relative sizes arbitrary, so proportions are taken only over the 85 tiles reaching an LWP of 7.5; two of those lie on the X, so the proportions are in effect autosomal. Confidence intervals are percentile bootstrap over contiguous groups of four tiles, 2000 resamples.
