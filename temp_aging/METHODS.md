...
The partitioning is carried out at every step. To describe genome-wide patterns we also summarize this partitioning and other statistics over 268 non-overlapping one-cM wide tiles spanning the euchromatic genome, the width chosen such that adjacent tiles stop being near duplicates of one another — for the Wald statistic adjacent tiles share $R^2 = 81\%$ of their variance at one cM, against $93\%$ at half that width and $60\%$ at twice it. Each tile is summarised by one step, the one within it whose Wald statistic, maximised over the four treatments, is largest. In several cases we summarize over the 85 tiles whose LWP exceeds 7.5, as patterns of variation for non-significant regions are not meaningful; two of those tiles lie on the X. Confidence intervals are percentile bootstrap over contiguous groups of four tiles, 2000 resamples.
