...
Read as a power calculation, the same expression says what an experiment of a given size can resolve, and it depends on how hard the selection is as much as on how much is sequenced. As run here, selecting the longest-lived $\sim$5% and sequencing 5,700 selected and 6,900 control flies per treatment, a window contributing 0.75% of phenotypic variance is detected at an LWP of 15 with 80% power. Holding the selected fraction at 5%, resolving 1% would take 3,800 selected and 4,600 control flies sequenced per treatment, drawn from 75,000 screened; resolving 0.5% would take twice that, and 0.25% four times.

Power goes as $n_{\mathrm{eff}}\bar{\imath}^{\,2}h^2$, so the selected fraction enters twice and is costly at both extremes. Selecting harder raises $\bar{\imath}$ and cuts the sequencing — resolving 1% from the top 1% needs only 2,300 flies sequenced per treatment — but they have to be found among 226,000 screened. Selecting more weakly lowers $\bar{\imath}$ and costs power directly: at the top 50% the intensity falls to 0.80 and 25,000 flies must be sequenced for the same result. Screening effort is least near a 20% selected fraction, sequencing effort falls monotonically with stringency, and the total is flattest between roughly 5 and 20%.
