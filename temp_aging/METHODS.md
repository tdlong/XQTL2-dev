...
The partitioning is carried out at every step. To describe genome-wide patterns we also summarize this partitioning and other statistics over 268 non-overlapping one-cM wide tiles spanning the euchromatic genome, the width chosen to match the mapping resolution of the synthetic population. Being an advanced intercross rather than a single-generation cross, unrecombined founder haplotypes are broken into short blocks and resolution is of the order of 0.5 to 2 cM (cite) rather than the 10 to 20 cM typical of a single-generation design. Of the natural widths of 0.5, 1, 2 and 5 cM, one cM is the narrowest that holds a typical peak whole: the two-LWP support intervals of the peaks in Table S1 have medians of 0.39 cM above an LWP of 15 and 0.76 cM between 7.5 and 15. It also leaves adjacent tiles no longer near duplicates of one another, sharing $R^2 = 81\%$ of their variance in the Wald statistic against $93\%$ at half that width and $60\%$ at twice it. Each tile is summarised by one step, the one within it whose Wald statistic, maximised over the four treatments, is largest. In several cases we summarize over the 85 tiles whose LWP exceeds 7.5, as patterns of variation for non-significant regions are not meaningful; two of those tiles lie on the X. Confidence intervals are percentile bootstrap over contiguous groups of four tiles, 2000 resamples.
