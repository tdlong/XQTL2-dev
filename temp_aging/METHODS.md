...
**Partitioning.** The partition asks how $\hat{h}^2$ itself varies across the design; the estimator is unchanged. Splitting the ten replicate contrasts in each treatment into two halves gives eight unbiased estimates of $\hat{h}^2$ at every step — two sexes $\times$ two diets $\times$ two halves — and it is these eight numbers that are decomposed. Writing $y_{sdh}$ for the estimate of heritability at some step in sex $s$, diet $d$ and half $h$, we can express
