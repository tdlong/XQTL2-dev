...
The above relationship can also be used the calculate the power of X-QTL experiments. To detect a step contributing 1% of phenotypic variance at an LWP of 15 with 80% power, selecting the longest-lived 5%, one need screen 75,000 flies per treatment. Halving the target heritability to 0.5% doubles the experiment to 150,000 individuals screened.

**Partitioning.** We split the ten replicate contrasts in each treatment into two halves to give eight unbiased estimates of $\hat{h}^2$ at every step — two sexes $\times$ two diets $\times$ two halves. (OK, why introduce a "y" here, what is gained?   We are decomposing heritability, and even then it is confusing as what then even is y^2 ... I mean we are really decomposing the variance in is h^2, like it seems like it is intentionally baroque) Writing $y_{sdh}$ for the estimate of heritability at some step in sex $s$, diet $d$ and half $h$, we can express
...
The eight estimates are thus re-expressed on an orthogonal basis: a level shared by all four sex $\times$ diet groups, a sex contrast, a diet contrast, a sex $\times$ diet contrast, and the four within-cell differences between halves. These carry $1+1+1+1+4 = 8$ degrees of freedom and partition $\sum y^2$ exactly. Each of the four single-degree-of-freedom terms equals $8\delta^2$ for a deviation $\delta$ on the heritability scale — $\delta = \bar{y}$ for the shared term, half the male–female difference for sex, half the SY20–SY10 difference for diet — so $\sqrt{\cdot/8}$ returns every term to that scale and makes them directly comparable.

(Again the above is awefully wordy.  Can't we just just say what the estimate is and be done with it, like this whole section is so wordy for now real reason.  We need to just state what we estimate and how we estimate it with precision).

The treatment terms carry one degree of freedom each. $SS_{\mathrm{rep}}$ carries four and is pure error: the two halves differ only in which replicates they contain. Each term is reported as $MS - MS_{\mathrm{rep}}$, with $MS_{\mathrm{rep}} = SS_{\mathrm{rep}}/4$, and on the heritability scale as $\operatorname{sign}(\cdot)\sqrt{|MS - MS_{\mathrm{rep}}|/8}$. Since the eight estimates are unbiased, so is each contrast, and a term whose true value is near zero falls negative about half the time; a component is taken as present only where its interval excludes zero. The shared term takes its sign from $\bar{y}$ rather than from the term itself, because $8\bar{y}^2$ is a square and a step with strongly negative $\hat{h}^2$ would otherwise read as a large positive shared effect.

(Is this the very first time you use the word tile?  You had introduced the term way early.  I removed the stuff about step and window and incorporated them where appropriate, but the concept of the tile seems to suddenly just appear out of nowhere.  I think we have to introduce (from the results we should be able to see why) we introduce this concept and then discuss it)

The partition is computed at every step, and Figure 1C shows the three components along the genome. Where nothing has responded all three are small and their relative sizes arbitrary, so genome-wide proportions are taken only over the 85 tiles reaching an LWP of 7.5, each read at its most significant step so that all eight estimates refer to the same place. Two of those tiles lie on the X, so the proportions are in effect autosomal. Confidence intervals are percentile bootstrap over contiguous groups of four tiles, 2000 resamples.

A *tile* is 1 cM of the genetic map cut from the start of each arm, 268 of them covering the 264 cM and 122 Mb of euchromatin. A statistic quoted for a tile is read at a single step, the one in that tile whose Wald statistic, maximised over the four treatments, is largest; a tile spans 65 to 101 steps and most of them carry no signal even where the tile does, so averaging over the tile would dilute a peak by an amount that depends on its width.
