...
this partitioning is still verbose and makes little sense.  Read it and think about it.  You introduce the y[sdh] notation.  h is a poor choice of a letter given you we are partioning h^2.  But at anyrate, you introduce this subscripting and then never use it again.  So I am back to why not just go right the the partion of h^2^2 = blah blah.  As those subscripts are clear.  I just think the way you decide the write things, means there is little advantage to introducing y and the sdh subscripts.  And again you introduce all sort of additional notation that is not necessary.  You define the partioning in one step, and then in the next just give you estimatorer of each term.  Done, why give all this intermediate stuff that doesn't matter.

**Partitioning.** We split the ten replicate contrasts in each treatment into two halves to give eight unbiased estimates of $\hat{h}^2$ at every step — two sexes $\times$ two diets $\times$ two halves. Writing $y_{sdh}$ for the estimate of $\hat{h}^2$ in sex $s$, diet $d$ and half $h$ the total sum of squares decomposes into four orthogonal single-degree-of-freedom terms and a four-degree-of-freedom error,
...
The partitioning is carried out at every step. To describe genome-wide patterns we also summarize this partitioning and other statistics for a series 268?? one-cM wide non-overlapping tiles throughout the euchromatic genome.  With the 1cM width chosen such that adjacent tiles stop being near duplicates of one another (R^2 = 90%) for Wald for example). Each tile is summarised by one step, the one within it whose Wald statistic, maximised over the four treatments, is largest. In several cases we summarize of the 85 tiles whose LWP exceeds 7.5, as patterns of variation for non-significant regions are not meaningful.
