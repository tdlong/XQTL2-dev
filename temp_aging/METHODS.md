...
Read as a power calculation, the same expression says what an experiment of a given size can resolve, and it depends on how hard the selection is as much as on how much is sequenced. As run here, selecting the longest-lived $\sim$5% and sequencing 5,700 selected and 6,900 control flies per treatment, a window contributing 0.75% of phenotypic variance is detected at an LWP of 15 with 80% power. Because power goes as $n_{\mathrm{eff}}\bar{\imath}^{\,2}h^2$, sequencing and screening trade against one another: to resolve 1% at the same threshold, selecting the top 1% would need only 2,300 flies sequenced per treatment but 226,000 screened, whereas selecting the top 20% would need 8,200 sequenced from 41,000 screened. Sequencing effort falls as selection is made more stringent, while screening effort is least near a 20% selected fraction and rises steeply on either side.
